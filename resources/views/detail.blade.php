			...
			<div class="col-md-8">
				...
				<div class="card">
					<h5 class="card-header">
						{{$detail->title}}
						<span class="badge btn-info">{{$detail->views}}</span>
					</h5>
				...
				<div class="alert alert-warning">
                 <strong>Note:</strong> you need to login to add comment. 
                 </div>
				<!-- Fetch Comments -->
				<div class="card my-4">
					<h5 class="card-header">Comments <span class="badge btn-info">{{count($detail->comments)}}</span></h5>
					<div class="card-body">
						@if($detail->comments)
					...
					<div class="list-group list-group-flush">
						@if($popular_posts)
							@foreach($popular_posts as $post)
								<a href="{{url('detail/'.Str::slug($post->title).'/'.$post->id)}}" class="list-group-item">{{$post->title}} <span class="badge btn-info ">{{$post->views}}</span></a>
							@endforeach
						@endif
					</div>
				</div>
